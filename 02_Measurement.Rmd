...
cov_cor_P <- paste0("R = ", round(cov_cor_R,2)[1], ", (", round(cor.test(cov_cor[,1],cov_cor[,2])$p.value,3)[1], ")")
...
cor1 <- df %>%
  ...
  subset(select = c(Coverage_pct_LF, Work_Inj_Cov_LF, diff_sied_geip, Sick_Cov_LF, Sick_Cov_Pop, gdppc_k, pop, lfp_total, lfp_female, Dis_RR, replacement_rate_perm, replacement_rate_temp, labor_workinjury_firstlaw, labor_workinjury_firstins)) %>%
cor(., use = "pairwise.complete.obs") %>%
  round(.,3)
...
### Fig 3. Replacement Rates GEIP & SIED. RR Temp, Single Worker
...
```{r gen}
df %>%
ggplot(aes(replacement_rate_temp/100, RR_Sing)) +
  geom_smooth(method = "lm", color = "#472D7BFF", size = 0.4) +
  stat_cor(method = "pearson", 
           size = 2.5, 
           color = "#472D7BFF",
...
### Fig 4. Replacement Rates GEIP & SIED. RR Temp, Average Single Worker

```{r gen}
df %>%
ggplot(aes(replacement_rate_temp/100, RR_Avg_Sing)) +
  geom_smooth(method = "lm", color = "#472D7BFF", size = 0.4) +
  stat_cor(method = "pearson", 
           size = 2.5, 
           color = "#472D7BFF",
...
### Fig 5. Replacement Rates GEIP & SIED. RR Temp, Family

Tracks family as well. 

```{r gen}
df %>%
ggplot(aes(replacement_rate_temp/100, RR_Fam)) +
  geom_smooth(method = "lm", color = "#472D7BFF", size = 0.4) +
  stat_cor(method = "pearson", 
           size = 2.5, 
           color = "#472D7BFF",
...
### Tbl 2. Correlation Matrix RRs

```{r}
cor2 <- df %>%
  mutate(diff_sied_gwip = replacement_rate_temp - RR_Avg_Sing) %>%
  subset(select = c(replacement_rate_temp, RR_Avg_Sing, diff_sied_gwip, Sick_Cov_LF, Sick_Cov_Pop, gdppc_k, pop, lfp_total, lfp_female, Dis_RR, replacement_rate_perm, labor_workinjury_firstlaw, labor_workinjury_firstins)) %>%
cor(., use = "pairwise.complete.obs") %>%
  round(.,3)
...
m11 <- lm(RR_Sing ~ replacement_rate_temp, data = df)
m12 <- lm(RR_Sing ~ replacement_rate_temp + gdppc_k, data = df)
m13 <- lm(RR_Sing ~ replacement_rate_temp + lfp_total, data = df)
m14 <- lm(RR_Sing ~ replacement_rate_temp + pop, data = df)
m15 <- lm(RR_Sing ~ replacement_rate_temp + labor_workinjury_firstins, data = df)

tab_model(m11,m12,m13,m14,m15, p.style = "stars", show.ci = F)



reg2 <- matrix(nrow = 7, ncol = 11)
colnames(reg2) <- c("Predictor", "m11_b", "m11_p", "m12_b", "m12_p", "m13_b", "m13_p", "m14_b", "m14_p", "m15_b", "m15_p")
reg2[,1] <- c("Intercept", "Temp_Replacement_Rate", "GDPpc", "LFP", "Population", "First Law", "R2")
reg2[1:2,2:3] <- round(summary(m11)$coefficients[,c(1,4)],3)
reg2[1:3,4:5] <- round(summary(m12)$coefficients[,c(1,4)],3)
reg2[c(1,2,4),6:7] <- round(summary(m13)$coefficients[,c(1,4)],3)
reg2[c(1,2,5),8:9] <- round(summary(m14)$coefficients[,c(1,4)],3)
reg2[c(1,2,6),10:11] <- round(summary(m15)$coefficients[,c(1,4)],3)
  
# Valid predictions for Global North
df$RR_Sing_Pred <- predict.lm(m11, newdata = df)

# get correct r-squared
mcors <- df
mcors$m12_pred <- predict.lm(m12, newdata = mcors)
mcors$m13_pred <- predict.lm(m13, newdata = mcors)
mcors$m14_pred <- predict.lm(m14, newdata = mcors)
mcors$m15_pred <- predict.lm(m15, newdata = mcors)

mcors <- mcors %>%
  mutate(!is.na(RR_Sing)) %>%
  select(RR_Sing, RR_Sing_Pred, m12_pred, m13_pred, m14_pred, m15_pred) %>%
  cor(., use = "pairwise.complete.obs")
...
reg2[7,c(2,4,6,8,10)] <- round(mcors$R2,2)

reg2[is.na(reg1)] <- ""

write_csv(as.data.frame(reg2), here::here("results", "Tbl2.csv"))
...
The prediction is of very high quality.
...
df %>%
  ...
  ggplot() +
  geom_smooth(aes(replacement_rate_temp/100, RR_Sing_I), method = "lm", se = F, size = 1, color = "#472D7BFF") +
  geom_point(aes(replacement_rate_temp/100, RR_Sing_Pred_Only), color = "#B8DE29FF", size = 1.5) +
  geom_point(aes(replacement_rate_temp/100, RR_Sing), color = "#33638DFF", size = 1.5) +
  #geom_text_repel(aes(label = ISO3c), size = 2) +
  theme_classic()
...
```{r fullimp_cov}
# briefly mean impute lfp_total for prediction purposes
...
tab_model(m21,m22,m23,m24, show.ci = F, p.style = "stars")

df$x <- predict.lm(m22, newdata = df)

reg3 <- matrix(nrow = 5, ncol = 9)
colnames(reg3) <- c("Predictor", "m1_b", "m1_p", "m2_b", "m2_p", "m3_b", "m3_p", "m4_b", "m4_p")
reg3[,1] <- c("SIED_imputed", "GDPpc", "LFP", "First Law", "R2")
reg3[c(1,2),2:3] <- round(summary(m21)$coefficients[,c(1,4)],3)
reg3[c(1,2,3),4:5] <- round(summary(m22)$coefficients[,c(1,4)],3)
reg3[c(1,2,4),6:7] <- round(summary(m23)$coefficients[,c(1,4)],3)
reg3[c(1,2,3,4),8:9] <- round(summary(m24)$coefficients[,c(1,4)],3)

  

# get correct r-squared
mcors <- df
mcors$m1_pred <- predict.lm(m21, newdata = mcors)
mcors$m3_pred <- predict.lm(m23, newdata = mcors)
mcors$m4_pred <- predict.lm(m24, newdata = mcors)


mcors <- mcors %>%
  mutate(!is.na(Work_Inj_Cov_Pred)) %>%
  select(Work_Inj_Cov_Pred, m1_pred, x, m3_pred, m4_pred) %>%
  cor(., use = "pairwise.complete.obs")

mcors <- as.data.frame(mcors[2:5,1])
...
reg3[5,c(2,4,6,8)] <- round(mcors$R2,2)

reg3[is.na(reg3)] <- ""

write_csv(as.data.frame(reg3), here::here("results", "Tbl3.csv"))
...
Tested gini and gdppc, no effects

```{r fullim_gen}
m31 <- lm(RR_Sing_imputed ~ gini_disp, data = df)
m32 <- lm(RR_Sing_imputed ~ Region_factor, data = df)
m33 <- lm(RR_Sing_imputed ~ labor_workinjury_firstlaw*Region_factor, data = df)
m34 <- lm(RR_Sing_imputed ~ as.factor(Colonial_Legacy) + Region_factor, data = df)
m35 <- lm(RR_Sing_imputed ~ gdppc_k + lfp_total + Work_Inj_Cov_LF_imputed_full, data = df)

tab_model(m31,m32,m33,m34,m35, show.ci = F, p.style = "stars")
# Polynesia does not work as a single region, recode to micronesia for the imputation

df <- df %>%
  mutate(Region_factor = ifelse(Region_factor == "Polynesia", "Micronesia", Region_factor))

df$xx <- predict.lm(m32, newdata = df)

reg4 <- matrix(nrow = 5, ncol = 9)
colnames(reg3) <- c("Predictor", "m31_b", "m31_p", "m32_b", "m32_p", "m33_b", "m33_p", "m34_b", "m34_p", "m35_b", "m35_p")
reg3[,1] <- c("SIED_imputed", "GDPpc", "LFP", "First Law", "R2")
reg3[c(1,2),2:3] <- round(summary(m21)$coefficients[,c(1,4)],3)
reg3[c(1,2,3),4:5] <- round(summary(m22)$coefficients[,c(1,4)],3)
reg3[c(1,2,4),6:7] <- round(summary(m23)$coefficients[,c(1,4)],3)
...
df <- df %>%
  mutate(Region_factor = ifelse(ISO3c == "PYF", "Polynesia", Region_factor))

df <- df %>%
  mutate(RR_Sing_imputed_full = ifelse(is.na(RR_Sing_imputed), xx, RR_Sing_imputed)) %>%
  select(-xx)
...
gwip20 <- df %>%
  mutate(cow_code = countrycode(ISO3c, "iso3c", "cown"),
         year = 2020,
         workinjury_coverage_GEIP = round(Coverage_pct_LF,3),
         ...
         workinjury_replacement_rate_perm_2020 = replacement_rate_perm,
         workinjury_replacement_rate_temp_2020 = replacement_rate_temp,
         workinjury_duration_temp_2020 = duration_temp,
         workinjury_duration_perm_2020 = duration_perm) %>%
  select(ISO3c, cow_code, workinjury_replacement_rate_perm_2020, workinjury_duration_perm_2020, workinjury_replacement_rate_temp_2020, workinjury_duration_temp_2020, Region, global_south, Colonial_Legacy, workinjury_coverage_GEIP, workinjury_coverage_SIED_imputed, workinjury_coverage_SIED_imputed_full, workinjury_replacement_rate_single_SIED_imputed, workinjury_replacement_rate_single_SIED_imputed_full)

gwip <- read_csv(here::here("data", "gwip_v1.1.csv"))

gwip20 <- gwip %>%
  left_join(gwip20, by = "cow_code")

gwip20 <- gwip20 %>%
  mutate(labor_workinjury_firstlaw_sspw = as.numeric(labor_workinjury_firstlaw_sspw),
         labor_workinjury_firstins = as.numeric(labor_workinjury_firstins)) %>%
  select(country_name, cow_code, ISO3c, everything())
```
...
gwip20_labels <- gwip20_labels %>%
  mutate(label = case_when(
    ...
    variables == "workinjury_replacement_rate_temp_2020" ~ "The average replacement rate after a work-injury, from previous income for the first 6 months of incapacity to work in 2020 or latest available, source = GWIP 2020",
    ...
    variables == "workinjury_replacement_rate_perm_2020" ~ "The average replacement rate after a work-injury, for a permanent or apparently permanent disability for the first 12 months of incapacity, rate for 2020 or latest available, source = GWIP 2020",
...
gwip_desc <- gwip_desc[!(rownames(gwip_desc) %in% c("country_name", "ISO3c", "cow_code", "labor_workinjury_firstlaw_programtype", "Region", "Colonial_Legacy", "workinjury_duration_perm_2020", "workinjury_duration_temp_2020","labor_workinjury_firstnat_carriedover")),]
...
# for workshop
write_csv(subset(gwip20, select = c(cow_code, ISO3c, workinjury_coverage_SIED_imputed, workinjury_coverage_SIED_imputed_full, workinjury_replacement_rate_single_SIED_imputed, workinjury_replacement_rate_single_SIED_imputed_full)), here::here("data", "gwip20_work_injury.csv"))
```
