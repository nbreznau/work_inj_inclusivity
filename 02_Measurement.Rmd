...
```{r gen}
cov_cor <- subset(df, select = c(replacement_rate_temp, RR_Sing))
...
m35 <- lm(RR_Sing_imputed ~ as.factor(Colonial_Legacy) + Region_factor, data = df)


# Only here did I realize one can save directly as a .doc table
...
gwip20 <- gwip20 %>%
  mutate(colonial_history = case_when(
    cow_code %in% c(366, 367, 368, 371, 372, 373, 703, 344, 355, 370, 359, 349) ~ "Russian",
    .default = Colonial_Legacy)) %>%
  select(-Colonial_Legacy)
```
