...
We have 2 ILO variables for coverage, as they differ sometimes between the GEIP and WSPR, we take the average. WSPR must be divided by 10 as it is in percentage and GEIP in ratio.
...
A first issue relates to some incompatibilities of the SIED and the GEIP. Although the former is from 2020 and the latter from 2014, the legal coverage of the labor force should not be so different. It has not changed in most rich countries. The correlation is around 0.5. 
...
cor1 <- df %>%
  ...
  subset(select = c(Coverage_pct_LF, Work_Inj_Cov_LF, diff_sied_geip, Sick_Cov_LF, Sick_Cov_Pop, gdppc_k, pop, lfp_total, lfp_female, Dis_RR, workinjury_replacement_rate_perm_2020, workinjury_replacement_rate_temp_2020, labor_workinjury_firstlaw, labor_workinjury_firstins)) %>%
cor(., use = "pairwise.complete.obs") %>%
  round(.,3)
...
```{r gen}
cov_cor <- subset(df, select = c(workinjury_replacement_rate_temp_2020, RR_Sing))
...
df %>%
ggplot(aes(workinjury_replacement_rate_temp_2020/100, RR_Sing)) +
...
```{r gen}
df %>%
ggplot(aes(workinjury_replacement_rate_temp_2020/100, RR_Avg_Sing)) +
  geom_smooth(method = "lm", color = "#472D7BFF", size = 0.4) +
  stat_cor(method = "pearson", 
           size = 2.5, 
           color = "#472D7BFF",
...
```{r gen}
df %>%
ggplot(aes(workinjury_replacement_rate_temp_2020/100, RR_Fam)) +
  geom_smooth(method = "lm", color = "#472D7BFF", size = 0.4) +
  stat_cor(method = "pearson", 
           size = 2.5, 
           color = "#472D7BFF",
...
```{r}
cor2 <- df %>%
  mutate(diff_sied_gwip = workinjury_replacement_rate_temp_2020 - RR_Avg_Sing) %>%
  subset(select = c(workinjury_replacement_rate_temp_2020, RR_Avg_Sing, diff_sied_gwip, Sick_Cov_LF, Sick_Cov_Pop, gdppc_k, pop, lfp_total, lfp_female, Dis_RR, workinjury_replacement_rate_perm_2020, labor_workinjury_firstlaw, labor_workinjury_firstins)) %>%
cor(., use = "pairwise.complete.obs") %>%
  round(.,3)
...
m11 <- lm(RR_Sing ~ workinjury_replacement_rate_temp_2020, data = df)
m12 <- lm(RR_Sing ~ workinjury_replacement_rate_temp_2020 + gdppc_k, data = df)
m13 <- lm(RR_Sing ~ workinjury_replacement_rate_temp_2020 + lfp_total, data = df)
m14 <- lm(RR_Sing ~ workinjury_replacement_rate_temp_2020 + pop, data = df)
m15 <- lm(RR_Sing ~ workinjury_replacement_rate_temp_2020 + labor_workinjury_firstins, data = df)
...
df %>%
  ...
  ggplot() +
  geom_smooth(aes(workinjury_replacement_rate_temp_2020/100, RR_Sing_I), method = "lm", se = F, size = 1, color = "#472D7BFF") +
  geom_point(aes(workinjury_replacement_rate_temp_2020/100, RR_Sing_Pred_Only), color = "#B8DE29FF", size = 1.5) +
  geom_point(aes(workinjury_replacement_rate_temp_2020/100, RR_Sing), color = "#33638DFF", size = 1.5) +
    annotate("text", 
           x = 0.75,
           y = 0.57,
           label = "Original\nSIED values",
...
agg_png(filename = here::here("results", "Fig4.png"), res = 144, width = 750, height = 500)
...
df <- df %>%
  ...
  select(ISO3c, cow_code, workinjury_coverage_SIED_imputed, workinjury_coverage_SIED_imputed_full, workinjury_replacement_rate_single_SIED_imputed, workinjury_replacement_rate_single_SIED_imputed_full)

gwip21 <- read_csv(here::here("data", "gwip21.csv"))

gwip21 <- gwip21 %>%
  left_join(df, by = "cow_code") %>%
  ...
  select(country_name, cow_code, ISO3c, independence, labor_workinjury_firstlaw, labor_workinjury_firstins, labor_workinjury_first_fund, labor_workinjury_first_socins, labor_workinjury_first_socins_source, labor_workinjury_firstnat, labor_workinjury_firstnat_carriedover, labor_workinjury_firstlaw_programtype, labor_workinjury_firstlaw_sspw, labor_workinjury_firstlaw_whitecollar_fullcoverage, labor_workinjury_firstlaw_bluecollar_fullcoverage, labor_workinjury_firstins_bluecollar_fullcoverage, labor_workinjury_firstlaw_agriworkers_fullcoverage, workinjury_replacement_rate_perm_2020, workinjury_duration_perm_2020, workinjury_replacement_rate_temp_2020, workinjury_duration_temp_2020, Region, global_south, workinjury_coverage_GEIP, workinjury_coverage_WSPR, colonial_history, workinjury_coverage_SIED_imputed, workinjury_coverage_SIED_imputed_full, workinjury_replacement_rate_single_SIED_imputed, workinjury_replacement_rate_single_SIED_imputed_full)
```
...
```{r codes}
gwip21_labels <- as.data.frame(colnames(gwip21))

colnames(gwip21_labels) <- "variables"

gwip21_labels <- gwip21_labels %>%
  mutate(label = case_when(
    ...
    variables == "labor_workinjury_firstlaw_whitecollar_fullcoverage" ~ "The year that all white-collar ('commerical') workers were legally covered. This does not necessarily include owners of their own businesses, but often does. This is considered full as long as undertakings with 5 or more employees are covered.", 
    variables == "labor_workinjury_firstins_bluecollar_fullcoverage" ~ "The year that all blue-collar workers are covered with a system that qualifies as social insurance or in rare cases as employer liability with compulsory insurance to the point that this insurance is legally embedded and enforced by the government",
    variables == "labor_workinjury_firstlaw_agriworkers_fullcoverage" ~ "The is the year that agricultural workers are covered by the work-injury law. Minimum undertakings with at least 5 workers.", 
...
nonna_gwip <- as.data.frame(t(sapply(gwip21 , function( x ) sum( !is.na( x ) ) )))
...
```{r save}
write_csv(gwip21, here::here("data", "gwip21_ext.csv"))

write_csv(gwip21_labels, here::here("data", "gwip21_labels.csv"))
