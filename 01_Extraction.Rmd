...
**Note** there were major updates to the GWIP v2. Now we are at v2.1. This version already contains the coverage data from the ILO
...
```{r gwip}
gwip <- read_csv(here::here("data", "gwip21.csv")) %>%
...
To make things easier we have added both of these data to the GWIP in the file `gwip21.csv`, available in the `/data` folder of this project. Users can otherwise find the original data in the files as `EIIP.csv` and `Social_Prot_Report_Tbl4.2_2020`.
...
df_labels <- as.data.frame(colnames(df)) %>%
  mutate(variables = `colnames(df)`,
         labels = case_when(
           ...
           variables == "Colonial_Legacy" ~ "Colonial Legacy, source = https://gist.github.com/caluchko with author updates for smaller countries",
           .default = NA)) %>%
