...
```{r predict}
m1 <- lm(Work_Inj_Cov_LF ~ Coverage_pct_LF, data = df)
...
tab_model(m1,m2,m3,m4,m5, p.style = "stars", show.ci = F)
...
m15 <- lm(RR_Sing ~ replacement_rate_temp + lfp_total + labor_workinjury_firstins + Dis_RR, data = df)
...
m01 <- lm(Work_Inj_Cov_Pred ~ gdppc_k, data = df)
m02 <- lm(Work_Inj_Cov_Pred ~ gdppc_k + lfp_total_I, data = df)
m03 <- lm(Work_Inj_Cov_Pred ~ gdppc_k + labor_workinjury_firstlaw, data = df)
m04 <- lm(Work_Inj_Cov_Pred ~ gdppc_k + labor_workinjury_firstlaw + lfp_total_I, data = df)

tab_model(m01,m02,m03,m04, show.ci = F, p.style = "stars")

df$x <- predict.lm(m02, newdata = df)
...
```{r fullim_gen}
m21 <- lm(RR_Sing_imputed ~ gini_disp*Region_factor, data = df)
m22 <- lm(RR_Sing_imputed ~ Region_factor, data = df)
m23 <- lm(RR_Sing_imputed ~ labor_workinjury_firstlaw*Region_factor, data = df)
m24 <- lm(RR_Sing_imputed ~ as.factor(Colonial_Legacy) + labor_workinjury_firstlaw*Region_factor + as.factor(Colonial_Legacy), data = df)
m25 <- lm(RR_Sing_imputed ~ gdppc_k + lfp_total + Work_Inj_Cov_LF_imputed_full, data = df)

tab_model(m21,m22,m23,m24,m25, show.ci = F, p.style = "stars")
...
df$xx <- predict.lm(m23, newdata = df)
...
gwip20 <- df %>%
  mutate(cow_code = countrycode(ISO3c, "iso3c", "cown"),
         year = 2020,
         GEIP_Coverage = round(Coverage_pct_LF,3),
         SIED_workinjury_coverage_imputed = round(Work_Inj_Cov_LF_imputed,3),
         SIED_workinjury_coverage_imputed_full = round(Work_Inj_Cov_LF_imputed_full,3),
         SIED_RR_Sing_imputed = round(RR_Sing_imputed,3),
         SIED_RR_Sing_imputed_full = round(RR_Sing_imputed_full,3)) %>%
  select(ISO3c, cow_code, replacement_rate_perm, duration_perm, replacement_rate_temp, duration_temp, Region, global_south, Colonial_Legacy, GEIP_Coverage, SIED_workinjury_coverage_imputed, SIED_workinjury_coverage_imputed_full, SIED_RR_Sing_imputed, SIED_RR_Sing_imputed_full)
...
gwip20_labels <- gwip20_labels %>%
  mutate(label = case_when(
    ...
    variables == "replacement_rate_temp" ~ "The average replacement rate after a work-injury, from previous income for the first 6 months of incapacity to work in 2020 or latest available, source = GWIP 2020",
    variables == "duration_temp" ~ "Duration of the temporary benefits calculated in replacement_rate_temp, source = GWIP 2020",
    variables == "replacement_rate_perm" ~ "The average replacement rate after a work-injury, for a permanent or apparently permanent disability for the first 12 months of incapacity, rate for 2020 or latest available, source = GWIP 2020",
    variables == "duration_perm" ~ "Duration of benefits calculated in replacement_rate_perm, under the assumption that the work has or appears to have a permanent disability, source = GWIP 2020."
    variables == "SIED_workinjury_coverage_imputed" ~ "The SIED variable acovratl, coverage of workers under work-injury law as a percentage of the labor force with missing values imputed from ILO GEIP coverage data, source = SIED 2020 & GEIP 2020",
    variables == "SIED_workinjury_coverage_imputed_full" ~ "The SIED variable acovratl, coverage of workers under work-injury law as a percentage of the labor force with missing values imputed from ILO GEIP and GWIP variables, rate for 2020 or latest available, source = SIED 2020, GEIP 2020 & GWIP 2020",
    variables == "SIED_RR_Sing_imputed" ~ "The SIED variable azrr26si, replacement rate after a work-injury for the first 6 months for a standard net single worker, imputed from replacement_rate_temp, rate for 2020 or latest, source = SIED 2020 & GWIP 2020",
    variables == "SIED_RR_Sing_imputed_full" ~ "The SIED variable azrr26si, replacement rate after a work-injury for the first 6 months for a standard net single worker, imputed from replacement_rate_temp and socioeconomic/historical variables, rate for 2020 or latest, source = SIED 2020 & GWIP 2020"
  ))
...
gwip_desc <- gwip_desc[!(rownames(gwip_desc) %in% c("country_name", "ISO3c", "cow_code", "labor_workinjury_firstlaw_programtype", "Region", "Colonial_Legacy")),]
```
...
# for workshop
write_csv(subset(gwip20, select = c(cow_code, ISO3c, SIED_workinjury_coverage_imputed, SIED_workinjury_coverage_imputed_full, SIED_RR_Sing_imputed, SIED_RR_Sing_imputed_full)), here::here("data", "gwip20_work_injury.csv"))
```
